  ...
  class {
    ...
    init() {
      for (let o = this; ; ) {
        ...
        if (!o || o === cds.Service.prototype) break;
        ...
        for (let p in pds) {
          ...
          this[on](
            ...
            !element
              ...
              : (req, next) =>
                  element in req.data && handler.call(this, req, next)
          );
...
cds.extend(cds.Request).with(
  class {
    ...
    error(e) {
      if (e.target)
        e.target = e.target.replace(/\((\w+)UUID=([^,)]+)/g, `($1UUID='$2'`);
    ...
    get _target() {
      let { target } = this,
        [key] = this.params;
      if (key && this.path.indexOf("/") < 0) {
        //> .../draftActivate
        // deviate to draft?
        const { IsActiveEntity } = key;
        if (IsActiveEntity !== undefined)
          Object.defineProperty(key, "IsActiveEntity", {
